const zh = {
    ...
    editSetting: "编辑",
    configureSetting: "配置",
    healthSummary: "{passed} / {total} 项正常",
    deliveryMaintenance: "邮件恢复",
    deliveryMaintenanceDesc: "核对外发记录，恢复中断的收信。外发核对不会重新发送邮件。",
    manualDelivery: "手动判定未知外发",
    manualDeliveryDesc: "先在供应商后台核实结果，再统一处理未知记录。",
    cloudflareEmailBinding: "Cloudflare 发信",
    searchTable: "邮件搜索表",
    settingsCache: "设置缓存",
    diagnosticEmailTotal: "邮件总数",
    diagnosticSearchRows: "搜索记录",
    diagnosticAttempts: "外发记录",
    diagnosticUnknown: "结果未知 · UNKNOWN",
    diagnosticPending: "等待确认 · PENDING_ACK",
    diagnosticDuration: "检查耗时",
    diagnosticIndex: "查询使用索引",
    diagnosticColumns: "缺失字段",
    diagnosticIndexes: "缺失索引",
    yes: "是",
    no: "否",
    ...
    setupTitle: '初次部署尚未完成',
    setupDesc: 'Cloud Mail 已成功启动，但运行所需的绑定、变量或数据库结构尚未就绪。',
    setupStatusTitle: '部署检查',
    setupConfigured: '已就绪',
    setupMissing: '未配置',
    setupCheckD1: 'D1 数据库绑定',
    setupCheckKv: 'KV 命名空间绑定',
    setupCheckDomain: '邮箱域名变量',
    setupCheckAdmin: '管理员邮箱变量',
    setupCheckSecret: '初始化密钥',
    setupCheckDatabase: '数据库结构',
    setupCheckAdminAccount: '管理员账户',
    setupInitRequired: '数据库尚未初始化',
    setupInitCommand: '在安全终端中执行以下命令：',
    setupAdminRequired: '数据库已就绪，还需要创建管理员账户',
    setupAdminCommand: '在安全终端中执行以下管理员创建命令：',
    setupCopyCommand: '复制命令',
    setupCopySuccess: '设置命令已复制',
    setupCommandHint: '复制到 PowerShell 执行。命令会以隐藏输入方式询问初始化密钥，不会把真实值写入命令历史；页面不会读取或保存密钥。',
    setupAdminCommandHint: '复制到 PowerShell 执行。命令会以隐藏输入方式询问初始化密钥和 6–30 位管理员密码，并安全生成 JSON；页面不会读取或保存这些值。',
    setupBindingsFirst: '请先在 Cloudflare 中补齐标记为“未配置”的绑定或变量，然后重新部署或检测。',
    setupRefreshStatus: '重新检测',
    setupMaintenanceHint: '维护中心需要管理员登录。首次部署请依次使用受密钥保护的 /api/init 和 /api/init/admin。',
    healthIssues: '健康检查',
    safeRepair: '安全修复',
    ...
    recoverReceive: '恢复卡住的收信',
    receiveRecoverResult: '本次处理 {scanned} 封，其中 {resolved} 封已处置完成；积压 {before} 封，剩余 {after} 封（每次最多 {batch} 封，可重复点击）',
    ackUnknownDelivery: '未知外发判为已发',
    failUnknownDelivery: '未知外发判为失败',
    unknownDeliveryConfirm: '仅在已于发信供应商后台核实真实结果后执行。该操作会把当前所有未知状态的外发投递统一改判；判为失败后发件人可重新发送。确认继续？',
    unknownDeliveryResult: '扫描 {scanned} 条，改判 {resolved} 条',
    repairConfirm: '该操作只会执行幂等安全修复，不会删除真实邮件、用户或附件数据。确认继续？',
    repairSuccess: '修复完成',
    codeMaintenance: '验证码维护',
    codeMaintenanceDesc: '按当前本地规则重算验证码字段，只修改邮件的验证码标记，不会删除邮件正文、用户或附件。',
    rescanCodes: '重新扫描验证码',
    cleanFalseCodes: '清理误判验证码',
    clearStaleCodes: '清除过期验证码',
    ...
    freshCode: '近 15 分钟',
    staleCode: '更早',
    searchCodeDesc: '搜索验证码、发件人、主题或邮箱',
    codeCenterHint: '点击卡片复制，“详情”查看原邮件。接收 15 分钟后自动隐藏；实际有效期以邮件为准。',
    copied: '已复制',
    codeExpiresIn: '约 {minutes} 分钟后隐藏',
    codeExpiredHint: '已超出展示时间，可查看原邮件',
    codeExpiredHidden: '已隐藏',
    loadMore: '加载更多',
    noVerificationCodeFound: '没有任何验证码',
    noMoreData: '没有更多数据了',
    ...
    blackList: '邮件黑名单',
    codeRecognition: 'AI兜底识别',
    codeRecognitionRules: 'AI兜底规则',
    codeRecognitionRulesDesc: '本地规则会自动识别常见验证码；这里只限制AI兜底识别范围，为空则AI兜底全部邮件',
    oss: '对象存储',
    osDomain: '访问域名',
    ...
    clearTurnstileKeyConfirm: '确认清空 {key} 吗？',
    ...
    localDrafts: '本地草稿',
    localDraftsMsg: '草稿保存在当前浏览器中，可在此清空当前账号的本地草稿及附件',
    clearLocalDrafts: '清除本地草稿',
    clearDraftsConfirmMsg: '确认清空当前账号在本浏览器中的所有草稿吗？此操作不可恢复',
    clearDraftsSuccessMsg: '本地草稿已清空',
    clearDraftsFailMsg: '清除本地草稿失败',
    ...
    listLoadFailed: '加载失败，请重试',
    retry: '重试',
    attTooLargeMsg: '附件 {name} 超过 {size}MB 大小限制',
    attTotalTooLargeMsg: '附件总大小超过 {size}MB 限制',
    recipientLimitMsg: '最多添加 {count} 个收件人',
    attCountLimitMsg: '最多添加 {count} 个附件',
    contentTooLargeMsg: 'HTML 和纯文本正文合计超过 {size}MB 限制',
    message: '邮件详情',
    language: '系统语言',
    ...
    backgroundWarning: '本地背景会自动压缩，避免拖慢登录页面',
    backgroundUploadHint: '原图 {size}；保存时会限制到 {width}×{height}，并压缩到 {max} 以内',
    backgroundCompressed: '背景已从 {before} 压缩到 {after} 并保存',
    backgroundFormatError: '请选择 JPG、PNG 或 WebP 图片',
    backgroundSourceTooLarge: '原图不能超过 {max}',
    backgroundPixelsTooLarge: '图片像素尺寸过大，请先缩小后再上传',
    backgroundProcessFailed: '背景图片处理或上传失败',
    rulesVerify: '规则',
